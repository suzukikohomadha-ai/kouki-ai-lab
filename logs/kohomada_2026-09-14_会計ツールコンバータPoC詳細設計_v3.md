...
- アオイの監査結果（v1・v2 とも PASS WITH CONDITIONS）および仕様変更 A-1/A-2 の方針決定は、秘書アイ経由で受領した指示に基づく。v3 の5点の修正指示も同様。
...
- **（v3）除却・売却仕訳を `DEPRECIATION` から外すべきか、`depreciationAccounts` の既定に「減価償却累計額」を含めるべきか（リスク10。ミナに見解依頼中）**
...
6. `fixedAssets` の既定：`warn`（減価償却仕訳も出力）か `exclude`（減価償却仕訳を除外）か。除却・売却仕訳の扱い（リスク10）はミナの見解を待って決める
...
1. **ノヴァ**：本v3との差分確認（`convert(input, opts)` 同期・E000時の空 Dataset/Report・`applyMappings` 同期・`verifyConfig` の `options` 値域検査・fixtures 命名・T6 への除却仕訳ケース追加・T11 期待値）。`depreciationAccounts` の既定はミナの見解が出るまで変更しない。
2. **ミナ**：リスク10（除却・売却仕訳と `DEPRECIATION` 判定、`depreciationAccounts` の既定）について会計上の見解（依頼中）。
...
7. **メイ**：ミナの見解を受けて `DEPRECIATION` 判定の既定を v4 に反映。
