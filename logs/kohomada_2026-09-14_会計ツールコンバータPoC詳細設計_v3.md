# 会計ツールコンバータ PoC（フェーズ0）詳細設計書 v3 ― CSVベース・ローカル完結型
...
- 基準日：2026-09-14
- ステータス：v3 Draft。v2 に対するアオイの再監査（PASS WITH CONDITIONS）で指摘された5点 (a)〜(e) と、ミナの会計見解を受けた減価償却仕訳の除外判定の変更 (f) を反映した版。他は v2 を踏襲。**本書の記載＝実装済みではない**（evidence-policy 原則13。実装状況はノヴァの報告を正とする）
- 前版：`logs/kohomada_2026-09-14_会計ツールコンバータPoC詳細設計_v2.md`（上書きせず保存）、v1 も同様に保存
...
| # | 区分 | 変更内容 | 該当節 |
|---|---|---|---|
| (a) | 整合 | リスク8 の補完策として挙げていた「レポートの『期首日の伝票一覧』」は実装に存在しない（`report.md` の節は9つ：概要／科目別／税区分別／期首残高・除外伝票／未マッピング／取引先／警告／補助科目／確認依頼）。実在する手段（期首残高・除外伝票セクション＋`report_diagnostics.csv` の W006/W007 との突合）に改めた | リスク・注意点 8 |
| (b) | 整合 | fixtures の取引先名の記述を実態に合わせた。`例_` 接頭辞が付くのは「例_ABC商事」「例_A銀行」「例_B銀行」のみ。「サンプル商事株式会社」「（株）サンプル商事」「ダミー物産」「テスト工業有限会社」はプレースホルダー名のまま。T5 の例も差し替え | §7 |
| (c) | 整合 | `convert` は実装どおり `convert(input, opts)` の2引数・同期。E000 時は `null` ではなく空の `Dataset`／`Report` を返す。`applyMappings` は同期で `suggester` 引数なし（suggester 呼び出しは `convert` 側）。非同期化（`Promise<ConvertResult>`）はフェーズ1で行う | §2、§2.4、§8 |
| (d) | 追記 | `verifyConfig()` が `options` の4項目（`departments`／`tags`／`fixedAssets`／`openingBalances`）の値域も検査することを追記。参考掲載している v1→v2 変更表 #7 にも補記 | §3.8、参考表 #7 |
| (e) | 追記 | リスク10 を新設：`depreciationAccounts` に「減価償却累計額」を含むため、除却・売却仕訳も `DEPRECIATION` と判定され `exclude` で除外されうる、という懸念。→ (f) で対応済みに書き換え | リスク・注意点 10 |
| (f) | 仕様変更 | ミナの会計見解を受け、`fixedAssets:"exclude"` の除外判定を変更：`depreciationAccounts` 既定を `["減価償却費", "一括償却資産償却"]` とし「減価償却累計額」を判定キーから外す。`fixedAssetAccounts` に「減価償却累計額」を含める。除外は「`depreciationAccounts` の科目を含み、かつ他のすべての科目が `depreciationAccounts`∪`fixedAssetAccounts` に含まれる伝票」のみ。現金・預金・未払金・損益科目が混在する伝票は除外せず新警告 **W017** で残す。あわせて「freeeの台帳は減価償却仕訳のみ自動生成」という記述を訂正（除却・売却仕訳も生成する／しないを選択できる、とされる）。期中除却資産を台帳に含めた場合の挙動は `[未確認]` として §9 に追加 | §1.2、§2.3、§2.4、§3.1、§4.2、§7 T6、リスク10、§9 |

### 参考：v1 → v2 の変更点（v2 から転記。#7 を (d) に合わせて補記）

| # | 区分 | 変更内容 | 該当節 |
|---|---|---|---|
| 1 | 仕様変更 A-1 | 期首残高検出：キーワード照合は摘要・メモのみ、単独「繰越」を除外。科目ベース判定は `openingBalanceCounterAccounts` と期首日の AND のみ | §2.3、§3.1 |
| 2 | 仕様変更 A-2 | `fixedAssets:"exclude"` の除外対象は `DEPRECIATION` のみ。`FIXED_ASSET` は W003 警告のみで除外しない | §1.2、§2.4、§4.2 |
| 3 | 整合 | `convert()` の非同期化（**v3 で撤回。PoCは同期、非同期化はフェーズ1**） | §2 |
| 4 | 整合 | `JournalLine.tagsRaw` 追加 | §2.3 |
| 5 | 整合 | `E000`（設定検証エラー）追加 | §2、§3.8、§4.1 |
| 6 | 整合 | `confirmed:false` 使用は `--dev` でも W012 | §3.3、§4.2 |
| 7 | 整合 | `verifyConfig()` にグループ化戦略名・`amountMode` の値域検査を追加。**（v3補記）`options` の `departments`／`tags`／`fixedAssets`／`openingBalances` の値域検査も含む** | §3.8 |
| 8 | 整合 | `run.json` に `input` を含める | §5 |
| 9 | 整合 | `RuleSuggester` は「フェーズ1前半。PoCでは Noop のみ」に統一 | §6.3、§8 |
| 10 | 体裁 | §9・§10 の節番号付与 | §9、§10 |
| 11 | 体裁 | 「確認済み事実」→「先行資料からの引用（本文未検証）」 | 同節 |
| 12 | テスト | T9 fixture を `mf/normal.csv`（BOM付き）＋`mf/utf8-nobom.csv` に修正 | §7 |
| 13 | テスト | fixtures の取引先名に `例_` 接頭辞（**v3 で実態に合わせて限定**） | §7 |
| 14 | 承認・未確認 | README 追記事項を §9・§10 に反映 | §6.5、§9、§10 |
| 15 | 技術 | `encoding.ts` の `Buffer` 依存を明記 | §6.1、§6.2 |
...
- freeeは固定資産台帳に登録すると減価償却仕訳を自動生成するため、台帳インポートと減価償却費を含む仕訳インポートを両方行うと二重計上の恐れがあるとされる〔ミナ整理・freee公式ヘルプの要約〕。**（v3 訂正）**加えて、除却・売却を台帳に登録した場合も、除却・売却仕訳を自動生成する／しないを選択できるとされる〔ミナ確認・freeeヘルプ「固定資産を除却する」「【法人】固定資産の売却を記帳する」。本文は本セッションでは未取得〕。したがって「台帳が自動生成するのは減価償却仕訳のみ」という v2 までの前提は不正確。
...
| 減価償却仕訳（`DEPRECIATION`） | **検出して警告 W004**（既定 `warn`）。`fixedAssets:"exclude"` で**出力から除外**するのは、伝票内の科目が `depreciationAccounts`∪`fixedAssetAccounts` だけで構成される「純粋な償却仕訳」のみ。現金・預金・未払金・損益科目が混在する伝票は除外せず **W017** | freee台帳自動生成仕訳との二重計上リスク対応（A-2＋(f)）。境界線は顧問先の会計方針で変わる `[要税理士確認]`（リスク10） |
...
`src/core/` は純関数（ファイル・ネットワーク・`process` に触れない）。最上位は次の1関数（**v3：実装に合わせ2引数・同期**）：

```ts
export function convert(input: ConvertInput, opts?: ConvertOptions): ConvertResult;   // 同期・副作用なし
...
export interface ConvertOptions {
  ...
  suggester?: MappingSuggester;      // §8。省略時 NoopSuggester。PoCでは同期の Noop のみ
}
...
- `convert()` は最初に `verifyConfig(profile)`（§3.8）を実行し、失敗時は**例外を投げず `E000` 診断を返して終了**する（`dataset.entries=[]`、`report` は空集計。CLIが終了コード2に変換）。
- `suggester` の呼び出しは `convert()` 側で、`applyMappings` の結果から未マッピング一覧を組み立てた後に一度だけ行う（§2.4）。
- **非同期化はフェーズ1で行う**：`LlmSuggester`（`suggest(): Promise<…>`）を導入する時点で `convert` を `Promise<ConvertResult>` に変更する。PoCの `NoopSuggester` は同期で扱う。
...
- **実装上の注意**：`iconv-lite` は Node の `Buffer` を前提とするため、`encoding.ts` は Node 環境依存になる。`core` の他モジュールは `encoding.ts` に依存せず `string` を受け取る設計とし、ブラウザ化時は `encoding.ts` のみ差し替える（§6.1）。
...
**フラグ付与（A-1 反映）**。正規表現・リストは `options.detectors` に外出し：
...
- `OPENING_BALANCE`（期首残高）は次の **いずれか** で付与する：
  ...
  - (b) 伝票日付が `fiscalYear.start` と一致し、**かつ** いずれかの行の科目が `openingBalanceCounterAccounts`（既定 `["元入金", "資本金", "繰越利益剰余金", "開始残高", "期首残高"]`）に含まれる。日付条件と科目条件は**両方必要**（AND）。
- `DEPRECIATION`（(f) 反映）：いずれかの行の科目が `depreciationAccounts`（既定 `["減価償却費", "一括償却資産償却"]`）に含まれる。**「減価償却累計額」は判定キーに含めない**（除却・売却仕訳にも必ず現れるため。ミナ見解）。
- `FIXED_ASSET`：いずれかの行の科目が `fixedAssetAccounts`（既定 `["減価償却累計額", "建物", "建物附属設備", "構築物", "機械装置", "車両運搬具", "工具器具備品", "土地", "ソフトウェア", "一括償却資産", "リース資産"]`。事務所の科目表に合わせて編集する）に含まれる。「減価償却累計額」はここに含める。
- **除外判定の補助属性 `pureDepreciation: boolean`**（(f) 新設）：`DEPRECIATION` かつ、伝票内の**すべての**行の科目が `depreciationAccounts`∪`fixedAssetAccounts` に含まれるとき `true`。現金・預金・未払金・損益科目（固定資産除却損・売却益等）が1行でも混在すれば `false`（期中売却の月割償却合算、少額減価償却資産の即時償却等が該当しうる）。
...
```ts
export function applyMappings(ds: Dataset, maps: Maps, opts: MappingOptions): { dataset: Dataset; diagnostics: Diagnostic[] }   // 同期。suggester 引数なし
```
...
7. **除外処理（A-2＋(f) 反映）**：`opts.openingBalances = 'exclude_and_report'` なら `OPENING_BALANCE` 伝票を出力対象から外し `I002`。`opts.fixedAssets = 'exclude'` なら **`DEPRECIATION` かつ `pureDepreciation === true` の伝票のみ**出力対象から外し `I002`。`DEPRECIATION` だが `pureDepreciation === false` の伝票（現金・預金・未払金・損益科目が混在）は**除外せず `W017`** を伝票単位で出す。**`FIXED_ASSET` のみの伝票（取得・除却・売却）はどの設定でも除外しない**。
8. **（v3）suggester の呼び出しは本関数では行わない**。`convert()` が本関数の診断から未マッピング一覧（`UnmappedItem[]`）を組み立て、`opts.suggester`（既定 Noop）を一度だけ呼び、提案を該当診断の `detail.suggestions` に添える。提案は自動適用しない。
...
// config/profile.sample.json
{
  ...
  "options": {
    ...
    "fixedAssets": "warn",             // "warn" | "exclude"（exclude は pureDepreciation な DEPRECIATION 伝票のみ除外）
    ...
    "detectors": {
      ...
      "fixedAssetAccounts": ["減価償却累計額", "建物", "建物附属設備", "構築物", "機械装置", "車両運搬具", "工具器具備品", "土地", "ソフトウェア", "一括償却資産", "リース資産"]   // 事務所の科目表に合わせて編集
    }
  }
}
```
...
v1 と同一構造（`entries: { <照合キー>: { freeeAccount, freeeSubAccount?, confirmed, source, note? } }`）。`confirmed:false` のエントリを使用した場合、**`--dev` の有無にかかわらず `W012`** を出す。`--strict` では error 扱い。
...
エクスポート時の入力ファイル名に顧問先名・事務所名を含めない（`in/journal.csv` のような無機質な名前にする。`run.json` にファイル名が記録されるため）。
...
- **（v3追記）`options` の4項目の値域**：`departments ∈ {drop, passthrough}`、`tags ∈ {drop, memo_tag}`、`fixedAssets ∈ {warn, exclude}`、`openingBalances ∈ {exclude_and_report, include_with_warning}`。範囲外は error（E000）
...
| コード | 内容 | 判定 |
|---|---|---|
| E000 | 設定検証エラー | `verifyConfig()` の error。`TODO_VERIFY` 残存・必須列欠落・不正な戦略名/amountMode/options値・税区分日付重複 |
...
| W004 | 減価償却仕訳（純粋） | `DEPRECIATION` かつ `pureDepreciation === true`。「freeeの固定資産台帳から自動生成される減価償却仕訳と二重計上になる恐れ」。`fixedAssets:'exclude'` なら**この伝票のみ**出力から除外して `I002`（A-2＋(f)） |
| **W017** | **（v3新設）減価償却科目と資金・損益科目が混在する伝票** | `DEPRECIATION` かつ `pureDepreciation === false`（現金・預金・未払金・固定資産除却損・売却益等が混在）。`fixedAssets:'exclude'` でも**除外せず出力に残す**。「期中売却の月割償却合算・少額減価償却資産の即時償却等の可能性。台帳インポートとの重複有無を伝票単位で確認」 |
...
| `run.json` | 実行メタ。`input: { fileName, encoding, hadBom, physicalRows, sha256 }` を含む。ほかに profile名・各設定ファイルのsha256・件数・所要時間・CLIオプション。**明細データ（金額・科目・摘要）は含めない** |

`report.md` の構成（9節）：①概要／②勘定科目別 借方・貸方合計（移行元 vs 変換後・差額・件数・備考。除外伝票による差額は備考に自動記載）／③税区分別合計／④期首残高・除外伝票セクション（減価償却除外分も含む）／⑤未マッピング一覧／⑥取引先セクション（W001/W002ペア）／⑦警告一覧／⑧補助科目割当サマリ／⑨確認依頼事項（固定文言：「本レポートは自動変換の下書きです。freeeへのインポートおよび勘定科目・税区分対応表の妥当性の最終確認は、会計事務所側の有資格者が行ってください。本ツールは税務・会計上の判断を行いません。」）。
...
- **将来のブラウザUI化**：`papaparse` はブラウザで動き、`core` の変換部は `string`／設定オブジェクトのみを受け取る。**ただし `src/core/encoding.ts` は `iconv-lite` 経由で Node の `Buffer` に依存するため、ブラウザ版では encoding 層を `TextDecoder('shift_jis')`＋Shift_JISエンコード可能な別実装に差し替える必要がある**。「そのまま流用」できるのは `encoding.ts` を除く `core` 各モジュール。差し替えを容易にするため、`core/index.ts` は `decodeBytes/encodeText` を注入可能（`ConvertOptions.codec?`）にしておく。
...
accounting-converter/
  ...
  src/
    core/
      index.ts        # convert(input, opts)（同期）のみ公開
      model.ts        # §2.3 の型
...
- `node --test`（`tsx` 経由）。fixtures＋`--dev` 相当のプロファイルで `convert()` を呼び、診断コード集合と集計値を `expected/*.json` と比較。
- fixtures はすべて**架空**。**（v3）取引先名の命名は実態どおり**：`例_` 接頭辞が付くのは「例_ABC商事」「例_A銀行」「例_B銀行」。「サンプル商事株式会社」「（株）サンプル商事」「ダミー物産」「テスト工業有限会社」はプレースホルダー名のまま使用する（いずれも実在の法人を指さない架空名。人名・住所・電話・登録番号を含めない）。金額は端数を含む。
...
| T6 | 固定資産・減価償却 | `mf/fixed-assets.csv` | 取得仕訳に W003、純粋な償却仕訳（減価償却費／減価償却累計額）に W004、除却仕訳（現金＋減価償却累計額＋固定資産除却損／車両運搬具）に W003（W004 は出ない）、期中売却で償却費と現金・売却益が混在する伝票に W017。`fixedAssets:'exclude'` で**W004 の伝票のみ**除外・I002、**取得・除却・W017 の伝票は出力に残る**（A-2＋(f)） |
...
| T11 | 設定検証 | `config/*.sample.json` | `verifyConfig()` が TODO_VERIFY・不正な `grouping.strategy`・不正な `amountMode`・不正な `options` 値を検出。`convert()` が E000 を返し例外を投げず、空の Dataset／Report を返す。`--dev` で W013、かつ `confirmed:false` 使用時に W012 |
...
export interface MappingSuggester {
  ...
  suggest(items: UnmappedItem[], ctx: { targetAccounts: string[]; targetTaxCodes: string[]; sourceSystem: SourceSystem }): Suggestion[];   // PoC：同期。フェーズ1で Promise<Suggestion[]> に変更し convert も非同期化
}
```

- `NoopSuggester`：常に空。**PoCで実装するのはこれのみ**。呼び出し元は `convert()`（§2.4 項8）。
- `RuleSuggester`（**フェーズ1前半・PoCでは実装しない**）：freee側科目一覧（社長が用意。TODO_VERIFY）との文字列類似で候補提示。外部通信なし。
- `LlmSuggester`（フェーズ1後半）：`sourceValue`（科目名・税区分名・補助科目名）と親科目名だけを送り、金額・摘要・取引先名・件数は送らない。`src/integrations/` に置き `core` は依存しない。**この時点で `suggest` を非同期化し、`convert` を `Promise<ConvertResult>` に変更する**。
...
8. A-1 の検出条件を絞ったことで、摘要に既定キーワードが無く相手科目も `openingBalanceCounterAccounts` に無い期首残高伝票は検出されない可能性がある。**（v3）補完策は実在する手段に限定する**：`report.md` ④「期首残高・除外伝票セクション」で除外された伝票を確認し、あわせて `report_diagnostics.csv` を W006（会計期間外）・W007（期首残高）で絞り込んで、専門家が期首日付（`fiscalYear.start`）の伝票と突合する運用で補う。「期首日の伝票一覧」という独立した節はレポートに存在しない。
9. A-2 により、取得仕訳を通常仕訳としてインポートした後に固定資産台帳も別途インポートする場合、台帳側の登録方法（取得仕訳との紐付け）はfreeeの仕様に依存し未確認。W003 の文言で確認を促す。
10. **（v3・(f) で対応済み）** v2 までの `depreciationAccounts` に「減価償却累計額」を含む判定では、除却・売却仕訳（例：借方 現金＋減価償却累計額＋固定資産除却損／貸方 車両運搬具）も `DEPRECIATION` と判定され `exclude` で除外されうる懸念があった。ミナの会計見解を受け、(i) 「減価償却累計額」を `depreciationAccounts` から外して `fixedAssetAccounts` へ移し、(ii) 除外対象を「`depreciationAccounts` の科目を含み、かつ他のすべての科目が `depreciationAccounts`∪`fixedAssetAccounts` に含まれる伝票」（`pureDepreciation`）に限定し、(iii) 資金・損益科目が混在する伝票は W017 で残す、という規則に変更した（§2.3・§2.4・§4.2）。**残るリスク `[要税理士確認]`**：この境界線は顧問先の会計方針（直接法／間接法、少額減価償却資産の特例・一括償却資産の適用、期中売却時の償却計上方法）によって変わるため、既定リストと除外規則が当該顧問先に適合するかは、移行のたびに税理士が確認する必要がある。ツールは既定を提供するのみで、方針の妥当性は判断しない。
11. **（v3新設）freee 台帳の自動生成範囲の訂正に伴う論点**：freee は除却・売却を台帳に登録した場合も除却・売却仕訳を自動生成する／しないを選択できるとされる（ミナ確認・本文未取得）。本書は「移行時点で在籍する資産のみ台帳に入れ、過去の除却・売却仕訳は通常仕訳として移行する」前提で W003 のみ（除外なし）としている。期中に除却した資産まで台帳に含める運用をとる場合の挙動は `[未確認]`（§9）。W003 の文言に「台帳へ除却・売却を登録する場合は自動生成の設定を確認」を含める。
...
- アオイの監査結果（v1・v2 とも PASS WITH CONDITIONS）および仕様変更 A-1/A-2 の方針決定は、秘書アイ経由で受領した指示に基づく。v3 の修正指示 (a)〜(f) も同様。
- (f) の会計上の根拠（「減価償却累計額」が除却・売却仕訳にも現れること、freee 台帳が除却・売却仕訳の自動生成を選択できること）はミナの見解に基づく。ミナが参照した freeeヘルプ「固定資産を除却する」「【法人】固定資産の売却を記帳する」の本文は本セッションでは未取得であり、メイ自身は確認していない。
- `convert` のシグネチャ・`report.md` の節構成・fixtures の命名は、アイ経由で伝えられたノヴァの実装状況に基づく（メイ自身はコードを読んでいない）。
...
- **（v3）freee の固定資産台帳に、移行期中に除却・売却した資産まで登録した場合に、除却・売却仕訳の自動生成と通常仕訳インポートがどう重複するか**（ミナ確認によれば生成する／しないを選択できるとされるが、本文未取得。リスク11）
- **（v3）`depreciationAccounts`／`fixedAssetAccounts` の既定リストと `pureDepreciation` の除外規則が、顧問先の会計方針（直接法／間接法・特例適用）に適合するか** `[要税理士確認]`（リスク10）
...
1. **ノヴァ**：本v3との差分確認（`convert(input, opts)` 同期・E000時の空 Dataset/Report・`applyMappings` 同期・`verifyConfig` の `options` 値域検査・fixtures 命名・T11 期待値）。(f) の実装：`depreciationAccounts`／`fixedAssetAccounts` の既定変更、`pureDepreciation` 判定、W017、T6 の除却・期中売却ケース、W003 文言の追記。
2. **ミナ**：見解受領済み（(f) に反映）。残る `[要税理士確認]`（リスク10）と `[未確認]`（リスク11）は、社長のヒアリング先税理士への確認事項としてヒアリング質問リストへの追加を検討。
3. **アオイ**：v3 の再監査（v2 条件付き合格の条件5点が満たされたか）。
4. **社長**：§3.7 の転記と §9 の確認事項1〜8。
5. **リサ**：インボイス経過措置の期間を国税庁一次情報で確認し、§9 の未確認を解消。
6. **リョウ**：公式テンプレートの非配布運用の妥当性、README／レポート末尾の免責文言案。
7. **メイ**：ノヴァの (f) 実装結果とアオイの v3 監査結果を v4 に反映。
