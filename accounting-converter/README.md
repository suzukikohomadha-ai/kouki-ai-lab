...
- 本ツールは **データを外部に送信しません**（ネットワーク通信を行うコードを含みません。テストで機械的に検査しています）。
...
| 固定資産・減価償却 | 検出して警告（W003/W004）。`fixedAssets: "exclude"` で除外可 |
...
- 経過措置の切替日（`options.invoiceTransitionDates`、既定 `2026-09-30`）は現行制度に基づきます。税制改正による延長が法制化された場合は、`maps/taxcodes` のエントリと切替日を追記してください（コード改修は不要）。
