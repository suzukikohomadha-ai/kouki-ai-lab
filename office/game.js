...
(function () {
  ...
  const SLOTS = [
    { x: 200, y: 235 }, { x: 400, y: 235 }, { x: 600, y: 235 },
    { x: 200, y: 430 }, { x: 400, y: 430 }, { x: 600, y: 430 },
    { x: 200, y: 625 }, { x: 400, y: 625 }, { x: 600, y: 625 },
  ];
  const GATHER = { x: 400, y: 715 };                            // 「集合」演出の整列位置（ホワイトボード前）
  ...
  const APPS = [
    ...
    { e: "💱", x: 560, y: 120, label: "為替", type: "forex", lampKeys: [] },
    { e: "📰", x: 630, y: 120, label: "ニュース", type: "news", lampKeys: [] },
  ];
  const TRAY = { x: 770, y: 200 };                              // 社長室の決裁トレイ
  const GUIDE = { x: 140, y: 770 };                             // 秘書アイ（ガイド役）の定位置
  const BIZ = { x1: 730, y1: 23, x2: 968, y2: 49 };             // 売上ボード（上壁・お金の見える化）
  const BOARD = { x: 400, y: 773 };                             // タスクのホワイトボード（ワークエリア内）
  const PROPOSAL = { x: 565, y: 778 };                          // 提案箱（社員からの提案）
  ...
  class Office extends Phaser.Scene {
    ...
    buildFloor() {
      ...
      // ラグ（ゾーン）
      rug(375, 488, 570, 695, 0xd8d0bf, 0xefe9dc);      // ワークエリア（フロー：人とタスク）
      rug(885, 185, 330, 270, 0xc9bbe6, 0xece6f8);      // 社長室（決裁）
      rug(885, 598, 330, 495, 0xd9cba8, 0xf2ead6);      // 資料室（ストック：成果物と資料）
      ...
      plant(50, 815, 1.0); plant(690, 815, 0.9); plant(46, 330, 0.9); plant(1030, 620, 0.9);
    }
