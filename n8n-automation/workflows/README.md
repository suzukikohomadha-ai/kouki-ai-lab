...
| `PUB-004` | Summarize Japan regulatory and government news with Claude and distribute the digest | `workflows/draft/PUB-004_japan-regulatory-news-digest.json` | 2026-08-15、社長承認のもと実機登録・**実機テストで完全成功を確認**（実ID`jejKmWHPDWSUifDZ`、実行ID#251：RSS取得→フィルタ→AI要約→Slack投稿→Sheets記録まで全工程成功、詳細は設計書参照） | 静的検証（0エラー・警告6件＝Sticky Note孤立のみ）＋**実機End-to-Endテスト成功** | **頒布方針を社長確認済み：n8n Creator Hubで無料公開**（n8nplace等の有料販売は不採用）。残作業：テスト用の設定値（実RSS・`lookbackDays:90`・テスト用Slackチャンネル）をプレースホルダーへ戻す→`PUB-003`との機能重複を踏まえた提出可否の最終判断→`workflows/validated/`へ格上げ検討（Slack Botに`chat:write`・`channels:read`スコープが必要な旨を提出資料に明記） |
