...
| `AUTO-KHM-001`（内製版） | 同上（社内運用版） | `workflows/draft/AUTO-KHM-001_japan-regulatory-news-digest.internal.json`として新規作成（`draft`） | 未登録（ローカルのみ） | 静的検証のみ実施（0エラー・警告6件＝Sticky Note孤立のみ、レイアウト間隔不足は2026-08-15解消済み） | 実RSSソース3件（METI/法務省/内閣府、PDL1.0準拠を確認済み）を設定済み、AI呼び出しをAUTO-COM-001経由に変更済み。**残作業**：実配信先（Slackチャンネル・Gmail・Notion DB）の確定（要社長確認）→Credential作成・割当（要承認）→`n8n-quality-auditor`監査→本番登録（要承認） |
| `PUB-004` | Summarize Japan regulatory and government news with Claude and distribute the digest | `workflows/draft/PUB-004_japan-regulatory-news-digest.json` | **2026-08-15、社長承認のもと実機登録済み**（実ID`jejKmWHPDWSUifDZ`、`active:false`）。AI呼び出しをAnthropic Messages API実装へ更新済み | 静的検証のみ実施（0エラー・警告6件＝Sticky Note孤立のみ） | **頒布方針を社長確認済み：n8n Creator Hubで無料公開**（n8nplace等の有料販売は不採用）。残作業：Slack/Google Sheets/Anthropic Credentialの割当（n8n UI上で社長実施）→手動実行テスト→`PUB-003`との機能重複を踏まえた提出可否の最終判断→`workflows/validated/`へ格上げ検討 |
