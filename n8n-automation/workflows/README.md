...
| `AUTO-COM-003` | 汎用承認リクエスト作成サブワークフロー（ライブワークフィード・送信側） | `workflows/draft/AUTO-COM-003_generic-approval-request.json`（`draft`） | 未登録（ローカルのみ） | aoi-quality-auditor監査：PASS WITH CONDITIONS（LINE Broadcast配信リスクを指摘）→2026-08-16、社長がPush Message方式への切替を選択し対応済み。ただしPush方式切替後のJSONに対する`validate-workflow.mjs`・`check-secrets.mjs`再実行は未実施（[実行環境なしのため未テスト]、ジンの前回結果はBroadcast版に対するもの） | 『承認待ちタスク・DB』（Notion database）の新規作成が前提条件（未着手）。**Push Message宛先`approverLineUserId`（社長個人のLINE userId）の実値が未確定**（プレースホルダーのままの間はLINE送信自体をスキップする設計）。受信側（LINE返信→ルーティング）の一般化は技術検証のみ実施し未実装（`docs/cases/AUTO-COM-003/workflow-design.md`参照、`AUTO-CNT-002`本番Webhookロジックの改修を伴うため別案件として日程を切ることを推奨） |
