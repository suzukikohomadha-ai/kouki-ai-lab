...
| `PUB-004` | Summarize Japan regulatory and government news with Claude and distribute the digest | `workflows/draft/PUB-004_japan-regulatory-news-digest.json`として新規作成（`draft`、元のプレースホルダー版を名称変更のみで複製） | 未登録（ローカルのみ） | 静的検証のみ実施（0エラー・警告6件＝Sticky Note孤立のみ、レイアウト間隔不足は2026-08-15解消済み） | **頒布方針を社長確認済み：n8n Creator Hubで無料公開**（n8nplace等の有料販売は不採用）。`PUB-003`との機能重複を踏まえた提出可否の最終判断／実機登録・テスト（要承認）を経て`workflows/validated/`へ格上げ検討 |
